    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG memory updating...' : 'Updating...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing; refresh the graph later if needed.</span>
...
const showSimulationFinishedHint = ref(false) // show when simulation done
const wasSimulating = ref(false) // was simulating before
...
// Watch isSimulating to detect simulation end
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Simulating -> not simulating: show done hint
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle self-loop expand/collapse
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // reset expanded
}
...
const renderGraph = () => {
  ...
  // Process edges: count and index per node pair
  const edgePairCount = {}
  const selfLoopEdges = {} // self-loop edges by node
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop: one merged edge per node
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // self-loop details
        }
      })
      return
    }
    ...
    // Edge direction vs normalized (source < target)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Curvature: spread for multi-edge, straight for single
    let curvature = 0
    if (totalCount > 1) {
      // Spread curvature by edge count
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // Flip curvature if edge direction reversed
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation: node spacing by edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Distance by edge count (base 150, +40 per edge)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only (click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect real drag (move past threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart sim on first real drag
          d._isDragging = true
  ...
  simulation.on('tick', () => {
    ...
    linkLabels.each(function(d) {
      ...
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // no rotation
    })
    
    // Update edge label bg
  ...
  // Click empty area to close detail
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
