<template>
  ...
  >
    <!-- Background: tech grid (shown when there are projects) -->
    ...
    <div class="section-header">
      ...
      <span class="section-title">Manage Timeline by LOKI</span>
      <div class="section-line"></div>
    </div>

    <!-- Cards container (shown when there are projects) -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card description (simulation requirement) -->
      ...
      <Transition name="modal">
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Timeline replay divider -->
            <div class="modal-divider">
              <span class="divider-line"></span>
              <span class="divider-text">Timeline replay</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start simulation" and Step5 "Deep interaction" must be started while running; they do not support timeline replay.</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // Selected project (for modal)
let observer = null
let isAnimating = false  // Animation lock
let expandDebounceTimer = null  // Debounce timer
let pendingState = null  // Pending expand/collapse state
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Expand down, spacing from title
...
// Title from simulation requirement (first 20 chars)
...
// Format simulation_id (first 6 chars)
...
// Navigate to Graph build (Process)
const goToProject = () => {
...
// Navigate to Environment (Simulation)
...
// Load timeline/history projects
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, record state only
        ...
        // Debounce: 50ms expand, 200ms collapse
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Unlock and check pending state
          setTimeout(() => {
            isAnimating = false
            
            // After animation, check pending
    ...
    },
    {
      ...
      // rootMargin: need more scroll to expand
...
onMounted(async () => {
  ...
  // Init observer after DOM
  setTimeout(() => {
    initObserver()
  }, 100)
})

// Reload on activate (keep-alive)
onActivated(() => {
  loadHistory()
})
