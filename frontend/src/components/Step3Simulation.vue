          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- Fallback: unknown type or content -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // from Step2
  minutesPerRound: {
    type: Number,
    default: 30 // min per round
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: not started, 1: running, 2: done
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // actions (incremental)
const actionIds = ref(new Set()) // dedup action IDs
const scrollContainer = ref(null)

// Computed
// Chronological actions (newest at bottom)
...
// Format elapsed time from rounds and min/round
...
const doStartSimulation = async () => {
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // graph memory update
    }
...
// Track last round per platform for log
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes per platform
      ...
      // Fallback: platforms may report done before runner_status
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ All platforms finished')
        }
    ...
    console.warn('Get run status failed:', err)
  }
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('Get detail status failed:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ Start report failed: ${res.error || 'Unknown error'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ Start report error: ${err.message}`)
    isGeneratingReport.value = false
  }
}
