...
const loadReportData = async () => {
  try {
    addLog(`Load report: ${currentReportId.value}`)
    
    // Get report to get simulation_id
    ...
    if (reportRes.success && reportRes.data) {
      ...
      addLog(`Get report failed: ${reportRes.error || 'Unknown error'}`)
    }
  } catch (err) {
