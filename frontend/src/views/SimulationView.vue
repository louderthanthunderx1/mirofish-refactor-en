 ...
 * When user goes back from Step 3 to Step 2, assume they want to exit
 */
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          addLog(`Stop failed: ${closeRes.error || 'Unknown error'}`)
          // If graceful failed, force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Stop error: ${closeErr.message}`)
        // On error, try force stop
        await forceStopSimulation()
      }
    } else {
      // Not running but process may exist, check status
      const simRes = await getSimulation(currentSimulationId.value)
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Simulation still running, stopping...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
    // Check failure does not block flow
    console.warn('Check sim status failed:', err)
  }
}
