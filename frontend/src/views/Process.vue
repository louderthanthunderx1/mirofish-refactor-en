    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph (shown when data available) -->
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">Waiting for ontology</p>
            <p class="waiting-hint">Graph build will start when ontology is ready</p>
          </div>
          
          <!-- Building, no data yet -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After upload, LLM analyzes documents and generates an ontology (entity types + relation types) for simulation.
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Description</div>
                <div class="detail-content">
                  Based on the ontology, documents are chunked and Zep API builds the knowledge graph (entities and relations).
                </div>
              </div>
...
const ontologyProgress = ref(null) // Ontology progress
...
const goToNextStep = () => {
  // TODO: go to environment setup step
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Load project failed'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'Load project failed: ' + (err.message || 'Unknown error')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Start graph build failed'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Start graph build failed: ' + (err.message || 'Unknown error')
    buildProgress.value = null
  }
}
...
const renderGraph = () => {
  ...
  // Draw edges (with clickable transparent area)
