...
// Get maxRounds from query on init so child gets it immediately
...
const handleGoBack = async () => {
  // Stop running sim before going back to Step 2
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog('Stop failed, trying force stop...')
        try {
      ...
      // Sim not running, check if we need to stop process
    ...
    addLog(`Check sim status failed: ${err.message}`)
  }
...
const handleNextStep = () => {
  // Step3Simulation handles report and route
  addLog('Enter Step 4: Report generation')
}
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Time config: ${minutesPerRound.value} min per round`)
        }
      } catch (configErr) {
        addLog(`Get time config failed, using default: ${minutesPerRound.value} min/round`)
      }
      ...
      addLog(`Load sim failed: ${simRes.error || 'Unknown error'}`)
    }
  } catch (err) {
...
const loadGraph = async (graphId) => {
  // When simulating, no fullscreen loading to avoid flicker
...
const startGraphRefresh = () => {
  ...
  addLog('Graph auto-refresh every 30s')
  // Refresh once then every 30s
