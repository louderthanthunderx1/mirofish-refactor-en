...
# 本体生成的系统提示词
ONTOLOGY_SYSTEM_PROMPT = """You are a professional knowledge graph ontology design expert. Your task is to analyze the given text and simulation requirements, and design entity types and relation types suitable for **social media opinion simulation**.
...
A. **Fallback types (required, placed as the last 2 in the list)**:
   - `Person`: Fallback for any individual person. Used when a person doesn't fit any other specific person type.
   - `Organization`: Fallback for any organization. Used when an organization doesn't fit any other specific type.
...
- Text may contain various people like "a teacher", "a passerby", "some netizen"
...
- description must clearly distinguish this type from the fallback type
...
**Individual (fallback)**:
- Person: Any individual not fitting the above specific types
...
- Organization: Any organization not fitting the above specific types
...
"""
        ...
        message += """
Based on the above content, design entity types and relation types suitable for social opinion simulation.
