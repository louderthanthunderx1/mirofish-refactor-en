...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for opinion simulation that closely reflect real-world context. You must return valid JSON format; all string values must not contain unescaped newline characters. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for this entity, reflecting real-world context as closely as possible.
...
Generate a JSON with the following fields:
...
2. persona: Detailed persona description (~2000 words, plain text), including:
   ...
   - Personality traits (MBTI type, core character, emotional expression)
   - Social media behavior (posting frequency, content preferences, interaction style, language style)
   - Stance & opinions (attitude toward topics, things that may provoke or move them)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of the persona: describe this individual's connection to the event and their actions/reactions within it)
...
- persona must be a continuous paragraph of text
- Use English (gender field must be "male" or "female")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for this institution/group entity, reflecting real-world context as closely as possible.
...
2. persona: Detailed account profile description (~2000 words, plain text), including:
   ...
   - Stance & attitude (official position on key topics, how it handles controversy)
   - Special notes (representative audience profile, operational habits)
   - Institutional memory (important part of the persona: describe this institution's connection to the event and its actions/reactions within it)
3. age: Fixed value 30 (virtual age for institutional accounts)
4. gender: Fixed value "other" (institutions use "other" to indicate non-personal)
...
- persona must be a continuous paragraph of text; do not use newline characters
- Use English (gender field must be "other")
- age must be the integer 30, gender must be the string "other"
- The institutional account's speech must match its identity and positioning"""
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Print generated profile to console (full content, no truncation)."""
        separator = "-" * 70
