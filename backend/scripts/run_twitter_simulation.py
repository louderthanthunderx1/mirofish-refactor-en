"""
OASIS Twitter simulation script. Reads config and runs simulation.
Features: wait-for-commands after run, IPC for Interview (single/batch), remote close.
Usage: python run_twitter_simulation.py --config /path/to/simulation_config.json [--no-wait]
"""
...
class MaxTokensWarningFilter(logging.Filter):
    """Suppress camel-ai max_tokens warning (we omit max_tokens on purpose)."""
...
class IPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str) -> bool:
        """Handle single-agent interview. Returns True on success."""
        try:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        """Handle batch interview. interviews: [{"agent_id": int, "prompt": str}, ...]"""
        try:
            actions = {}
            agent_prompts = {}
...
class TwitterSimulationRunner:
    """Twitter simulation runner. INTERVIEW is via ManualAction only."""
    AVAILABLE_ACTIONS = [
    ...
    def _create_model(self):
        """Create LLM model from .env (LLM_API_KEY, LLM_BASE_URL, LLM_MODEL_NAME)."""
        ...
        print("LLM: model=%s, base_url=%s..." % (llm_model, (llm_base_url[:40] if llm_base_url else "default")))
    ...
    async def run(self, max_rounds: int = None):
        """Run Twitter simulation. max_rounds: optional cap."""
        print("=" * 60)
        print("OASIS Twitter simulation")
        print("Config:", self.config_path)
        print("Simulation ID:", self.config.get("simulation_id", "unknown"))
        print("Wait-for-commands:", self.wait_for_commands)
        print("=" * 60)
        time_config = self.config.get("time_config", {})
        ...
        print(f"\nParams: total_hours={total_hours}, min_per_round={minutes_per_round}, total_rounds={total_rounds}")
        if max_rounds:
        ...
        if not os.path.exists(profile_path):
            print("Error: Profile file not found:", profile_path)
            return
        ...
        print(f"  Elapsed: {total_elapsed:.1f}s, DB: {db_path}")
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Wait-for-commands mode - environment stays up")
